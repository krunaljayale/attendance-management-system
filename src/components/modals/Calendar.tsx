...
// Mock Data: Only 'holiday' is relevant for the Global Calendar now.
const GLOBAL_EVENTS: Record<string, 'holiday'> = {
  '2026-01-22': 'holiday',
  '2026-02-14': 'holiday',
  '2026-08-15': 'holiday',
  '2026-10-02': 'holiday',
  ...
  return (
    <div className="w-full bg-white dark:bg-[#1A0F1E] p-6 md:p-10 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm transition-colors duration-300">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-4">
        <div>
          ...
          <div className="flex items-center gap-4 mt-2">
            ...
            <p className="text-secondary dark:text-gray-400 text-lg font-medium min-w-[140px] text-center">
              Academic Year <span className="text-primary dark:text-white font-bold">{year}</span>
            </p>
        ...
        {/* Legend - Simplified to only Holiday & Selected */}
...
function MonthGrid({ monthName, monthIndex, year, selectedDate, onSelectDate }: MonthGridProps) {
    ...
    <div className="flex flex-col">
      ...
      <div className="grid grid-cols-7 mb-2 text-center">
        {DAYS_OF_WEEK.map((day, i) => (
          <div key={i} className="text-[10px] font-bold text-secondary dark:text-gray-600 uppercase">
            {day}
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7 gap-y-2 gap-x-1">
        {calendarDays.map((day, i) => {
          ...
          const isHoliday = GLOBAL_EVENTS[dateStr] === 'holiday';
          ...
          // Styling Logic: Only check for Holiday or Default
          let bgClass = "hover:bg-gray-100 dark:hover:bg-white/10 text-primary dark:text-gray-300";
          
          if (isHoliday) {
            bgClass = "bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400 font-bold";
          }
          
          if (isSelected) {
            // Selected overrides holiday visually to show "This is what I clicked"
            bgClass = "bg-primary dark:bg-white text-white dark:text-[#1A0F1E] shadow-md scale-110 font-bold z-10";
          }
